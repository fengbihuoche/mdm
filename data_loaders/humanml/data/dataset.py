...
class Text2MotionDatasetV2(data.Dataset):
    def __init__(self, opt, mean, std, split_file, w_vectorizer):
        ...
        min_motion_len = 5 if self.opt.dataset_name =='t2m' else 24  ####min motion
        ...
        for name in tqdm(id_list):
            try:
                motion = np.load(pjoin(opt.motion_dir, name + '.npy'))
                #print(len(motion)) ##################
                if (len(motion)) < min_motion_len or (len(motion) >= 40):
                    continue
                text_data = []
                flag = False
                with cs.open(pjoin(opt.text_dir, name + '.txt')) as f:
...
'''For use of training text-scene motion matching model, and evaluations'''
class Text2MotionDatasetV2_Scene(data.Dataset):
    def __init__(self, opt, mean, std, split_file, w_vectorizer,split):
        self.opt = opt
        self.w_vectorizer = w_vectorizer
        self.max_length =20
        self.pointer = 0
        ...
        with cs.open(split_file, 'r') as f:
            for line in f.readlines():
                id_list.append(line.strip())
        # id_list = id_list[:200]
        #id_list=random.sample(id_list,2)  #########when evaluate,only 60
        new_name_list = []
        length_list = []
        ...
        for name in tqdm(id_list):
            #print(name) #############
            try:
                origintrans=originmotions[index]["smpl"][0][0]  #transition of first frame
                motion = np.load(pjoin(opt.motion_dir, name + '.npy'))
                ...
                pointcloud=torch.tensor(pointcloud).unsqueeze(0) #1x5120x6
                pointtransformer=PT(num_point=5120, nblocks=2, nneighbor=8, num_class=256, input_dim=6)
                scene=pointtransformer(pointcloud).to(dist_util.dev()) ##1xd
                scene=torch.squeeze(scene)#d
                #print(scene.shape)
                pointcloud=scene.tolist()
                
                if (len(motion)) < min_motion_len or (len(motion) >= 40):
                    continue
                text_data = []
                flag = False
                #print(pjoin(opt.text_dir, name + '.txt')) #############
                with cs.open(pjoin(opt.text_dir, name + '.txt')) as f:
                    for line in f.readlines():
                        text_dict = {}
                        line_split = line.strip().split('#')
                        ...
                        if f_tag == 0.0 and to_tag == 0.0:
                            ...
                            try:
                                n_motion = motion[int(f_tag*20) : int(to_tag*20)]
                                if (len(n_motion)) < min_motion_len or (len(n_motion) >= 40):
                                    continue
                                new_name = random.choice('ABCDEFGHIJKLMNOPQRSTUVW') + '_' + name
                                while new_name in data_dict:
                                    new_name = random.choice('ABCDEFGHIJKLMNOPQRSTUVW') + '_' + name
    ...
    def __getitem__(self, item):
        idx = self.pointer + item
        data = self.data_dict[self.name_list[idx]]
        motion, m_length, text_list,scene = data['motion'], data['length'], data['text'],data["pc"]
        # Randomly select a caption
        text_data = random.choice(text_list)
        caption, tokens = text_data['caption'], text_data['tokens']
        ...
        # print(word_embeddings.shape, motion.shape)
        # print(tokens)
        return word_embeddings, pos_one_hots, caption, sent_len, motion, m_length,'_'.join(tokens),scene#########
    ...
    def __init__(self, opt, mean, std, split_file):
        ...
        with cs.open(split_file, 'r') as f:
            for line in f.readlines():
                id_list.append(line.strip())
        # id_list = id_list[:200]
        '''id_list=random.sample(id_list,60)  #####scene sample
        print(len(id_list))
        print("hhhhh")'''
        new_name_list = []
        length_list = []
...
class HumanML3D(data.Dataset):
    def __init__(self, mode, datapath='./dataset/humanml_opt.txt', split="train", **kwargs):
        ...
        opt.data_root = opt.data_root   ###############改
        opt.save_root = pjoin(abs_base_path, opt.save_root)
        opt.meta_dir = './dataset'
        opt.npy_root=opt.npy_root ############改
        self.opt = opt
        ...
        if mode == 'gt':
            ...
            # used by our models
            self.mean = np.load(pjoin(opt.npy_root, 'mean.npy')) ############改
            self.std = np.load(pjoin(opt.npy_root, 'std.npy')) ############改
...
class HumanML3D_Scene(data.Dataset):
    def __init__(self, mode, datapath='./dataset/humanml_opt.txt', split="train", **kwargs):
        self.mode = mode
        ...
        if mode == 'text_only':
            ...
            self.w_vectorizer = WordVectorizer(pjoin(abs_base_path, 'glove'), 'our_vab')
            self.t2m_dataset = Text2MotionDatasetV2_Scene(self.opt, self.mean, self.std, self.split_file, self.w_vectorizer,self.split)
            self.num_actions = 1 # dummy placeholder
